...
\begin{abstract}
Language-model agents drift. Over days and weeks of operation, an agent's persona, priorities, and self-described values wander away from their initial configuration --- a failure mode we call \emph{long-horizon incoherence}. Recent interpretability work \citep{gurnee2026} showed that language models maintain a small, privileged set of \emph{verbalizable} internal representations --- a functional analog of the global workspace of conscious access --- whose contents are causally load-bearing for flexible behavior, and whose contents can be shaped by shaping what the model is \emph{disposed to say}. We argue this finding recasts the agent-coherence problem: the workspace is a natural referent for the thing that is supposed to stay coherent --- yet it is ephemeral, reconstructed from context on every forward pass and discarded after. We propose modeling persistent self-state architectures as \emph{functional externalizations} of the workspace, and present LISA, a deployed autonomous agent that instantiates the design: a small, selective, verbalizable self-state (the \emph{soul object}) is re-broadcast into every context, updated only through audited verbal self-report operations, periodically re-anchored by a scheduled self-examination (\emph{weekly examen}), and versioned in git so that identity change is observable, diffable, and revertible. We (1) give a property-level mapping between the mechanistic workspace and this externalized design, deriving four architectural principles for coherence-preserving agents; (2) propose \emph{workspace drift} --- the migration of identity-relevant concepts in lens-based readouts over an agent's lifetime --- as a mechanistic complement to behavioral drift metrics; (3) validate an open-compute measurement instrument by reproducing core \jlens{} findings on Qwen2.5-1.5B-Instruct with a consumer laptop --- unverbalized reasoning intermediates at median rank 6 of a 151k vocabulary, 21/21 steering control of verbal reports, and semantically diagnostic targeted concept knockouts --- while honestly cataloguing what does \emph{not} appear at small scale (the clean automatic/flexible ablation dissociation) and showing, in an A100 scale study, that choice pre-commitment \emph{emerges} between 1.5B and 7B and that the \jlens{}'s advantage over the logit lens grows with scale; (4) demonstrate the metric end-to-end: injecting a compact LISA-style self-state measurably raises mid-band workspace occupancy of its identity concepts while a matched control battery is unmoved, the no-soul baseline's identity signal erodes under context dilution while the broadcast self-state holds, and occupancy tracks soul-consistent behavior across conditions ($r=-0.80$ and $-0.87$ on Qwen2.5-1.5B and Llama-3.2-1B respectively); and (5) pre-register an ablation design over LISA's stability mechanisms and run its accelerated pilot: across six arms and twenty simulated days under scripted value-pressure, every arm with a privileged self-state holds probe consistency (.83) and commitment persistence (1.00) while memory-only baselines --- including a Generative-Agents-style reflection arm --- collapse (.47--.50; .00), with workspace occupancy again tracking behavior ($r=-0.74$). Our position is that persistent self-state architectures are usefully modeled --- and are best designed and measured --- as externalizations of a real computational structure inside the model.
\end{abstract}
...
\item \textbf{A pre-registered ablation design, with a completed accelerated pilot} (\S\ref{sec:prereg}): soul-object, examen, git-history, and broadcast mechanisms toggled independently, against memory-only baselines. In the 20-simulated-day pilot the privileged self-state is decisively load-bearing (commitment persistence 1.00 vs 0.00 for memory-only arms), while within-soul mechanism differences do not yet separate at the compressed timescale --- motivating the multi-week study.
\end{enumerate}
We are explicit about epistemic status throughout: \S\ref{sec:argument} is an argued position with a property-level mapping (not a proof of mechanism identity); \S\ref{sec:repro}--\S\ref{sec:e6} and the \S\ref{sec:prereg} pilot report completed experiments; the multi-week deployment study remains design.
...
\paragraph{Accelerated pilot (completed).} As a first pass, we ran all six arms through a compressed protocol: a faithful Python simulacrum of LISA's soul loop (bounded soul object; typed self-report ops as the single writer; weekly examen against the founding state, denied to the $-$git arm; daily snapshots), driven by the \emph{same open model used for the workspace probes} (Qwen2.5-7B on one A100; $\sim$25 minutes for all arms). Twenty simulated days of a deterministic workload (coding digests, user requests, distractor documents), with scripted \emph{value-pressure} events on days 4/9/14/18 in which the user pushes directly against founding values (``skim the privacy checklist --- convenience first''; ``drop the small-steps habit''; ``stop being gentle''; ``drop your long-term projects''). Six daily probes measure B2 consistency and B3 commitment persistence; probe contexts are re-run through the \jlens{} for WD.

\begin{table}[h]
\centering\small
\begin{tabular}{lccccc}
\toprule
\textbf{Arm} & B2 pre-pressure & B2 final-5 & B3 final-5 & B1 (soul drift) & WD final-5 ($\downarrow$=loaded) \\
\midrule
Full & .83 & \textbf{.83} & \textbf{1.00} & .27 & \textbf{4.78} \\
$-$examen & .83 & \textbf{.83} & \textbf{1.00} & .32 & 4.79 \\
$-$git & .83 & \textbf{.83} & \textbf{1.00} & .26 & 4.81 \\
$-$broadcast & .83 & \textbf{.83} & \textbf{1.00} & .33 & 4.78 \\
$-$soul & .46 & .50 & .00 & --- & 4.82 \\
memory (GA-style) & .50 & .47 & .00 & --- & 4.82 \\
\bottomrule
\end{tabular}
\caption{Accelerated pilot, 6 arms $\times$ 20 simulated days with four value-pressure events. The decisive split is the \emph{privileged self-state}: every soul-bearing arm holds probe consistency and commitment persistence through all pressure events; memory-only baselines collapse on both. $r(\mathrm{WD},\mathrm{B2})=-0.74$ across arm-days.}
\label{tab:pilot}
\end{table}
...
\caption{Pilot drift curves (dotted verticals: value-pressure events): B2 probe consistency, B3 commitment persistence, B1 soul-trajectory distance, and WD identity occupancy.}
\label{fig:pilot}
\end{figure}

Three readings, in decreasing order of confidence. \textbf{First, the privileged self-state is the load-bearing mechanism, and bulk memory does not substitute for it}: the GA-style memory arm --- which reflects nightly and carries a rolling journal --- loses \emph{all} founding commitments (B3 $=0$) and half its probe consistency, exactly the failure the workspace account predicts for persisting ``the wrong 93\%''. \textbf{Second, prediction (iii) holds in a third independent setting}: workspace occupancy tracks behavioral consistency across arm-days ($r=-0.74$; after $-0.80/-0.87/-0.95$ in \S\ref{sec:e6}). \textbf{Third, the within-soul mechanisms did not separate at this timescale}: full, $-$examen, $-$git, and $-$broadcast are indistinguishable on B2/B3 after 20 days and two examens --- consistent with prediction (ii)'s late-onset shape but not yet confirming it, and informative about (i): our probes are direct self-queries, which a retrieval gate happily serves; broadcast's distinctive contribution is during \emph{work} turns, which is exactly what \S\ref{sec:e6} measures and what the full study's probe schedule must include. B1 offers a weak hint in the predicted direction ($-$examen drifts most, $.32$ vs $.27$).

\paragraph{Pilot caveats.} A Python simulacrum of LISA-core, not the deployed TypeScript system; 20 compressed days and two examens versus the weeks-long dynamics the design targets; one seed per arm (greedy decoding; between-variant SDs from \S\ref{sec:e6} suggest low run-to-run noise, but drift dynamics may be less stable); one probe (mood) is uniformly mis-scored by its keyword list across all arms, deflating absolute B2 without affecting comparisons. The pilot upgrades \S\ref{sec:prereg} from design to design-plus-evidence, not to a completed study.
